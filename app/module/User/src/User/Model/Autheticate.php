...
class Autheticate {
    ...
    public function __construct(\Zend\Session\Config\SessionConfig $sessionConfig = null, $sessionStorageName = '') 
    {
        
        //Initialize Authentication Service
        $this->auth = new AuthenticationService();
        
        // Set Session Configaration
        $this->sessionConfig = $sessionConfig;
        
        // Set Session Storage Name
        // Use 'users' instead of 'Zend_Auth'
        if ($sessionStorageName == '')
            $sessionStorageName = 'Zend_Auth';
    ...
    {
         if ($rememberMe) {
             //$this->_sessionConfig->rememberMe($time);
             //$this->_sessionConfig->setStorageOption('remember_me_seconds', 2419200);
             $config = array(
    ...
    public function setStorage($data = null, \Zend\Db\Adapter\Adapter $dbAdapter = null) 
    {
        ...
        if(isset($data->role_id)) {
            $adminRole = new \Admin\Model\AdminRole($dbAdapter);
            $roleInfo = $adminRole->getData($data->role_id);
            $data->role_code = isset($roleInfo[0]['role_code']) ? $roleInfo[0]['role_code'] : '';
        }
