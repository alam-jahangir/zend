...
class AccountController extends AbstractActionController 
{
	...
	public function uploadAction() {
        ...
        if ($request->isPost()) {
			...
			$video = 0;
			/*
        	$file = $this->params()->fromFiles('upload_video');
        	if (($file["type"] == "video/mp4" || $file["type"] == "video/mp3") && $file["size"] < 1048576) {
				...
				if (move_uploaded_file($file['tmp_name'], $directory.DS.$filename)) {
					$saveData['video_filename'] = 'user_'.$this->userinfo->id.'/'.$filename;
					$video = 1;
				}
		    } else {
				$message[] = $this->config['message']['video_invalid_msg'];
			}
	        */
			$validation = new \Application\Model\Validation();
            	...
            	if ($id = $dealerUpload->setData($id, $saveData)) {
		        	...
		        	if (isset($userSession->filelist)) {
						...
						foreach ($fileList as $file) {
							...
							$dealerUpload->setUploadGallery(0, $galleryData);
						}
					}
										
					// Save Dealer Upload product Cart Price
					$cost = \Application\Model\CostCalculation::get($saveData['recommanded_price'], count($fileList), $video);
            		$cartPrice = array(
						'upload_id' => $id,
	...
	public function uploadVideoAction() {
		if ( $this->getRequest()->isXmlHttpRequest() ) {
			...
			if (isset($_FILES)) {
				$allowed = array('mp4');
				$ext = getext($_FILES['upload_video']['name']);
				...
				if (in_array($ext, $allowed)) {
					if ($size < 2097152*20) {
						...
						$filename = round(microtime(true) * 1000).str_replace(array(' ', '/', DS, '*', '%', '$', '&'), '', $filename);
			            $filename = str_replace(
							array(' ','/',DS,'*','%','$','&','+','#','@','!','~','?','[',']','{','}',')','(','"',',',':',';',','), 
							'', 
							$filename
						...
						echo "File size more than <strong>20MB<strong>";
					}
				} else {
